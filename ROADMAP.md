...
## Phase B — Dashboard tab: see what's running (all NEW code, no wallet changes)

The user's top ask: when a strategy runs DRY or LIVE, its status/results must be visible.

- [ ] B1 🔧 **Strategy status board.** New file
...
- [ ] B2 🔧 **Engine panel on Dashboard.** Show `/engine/settings` (paused, caps) with the
      ...
      15s and vice versa.
- [ ] B3 🔧 **Activity tables on Dashboard.** Recent trades (last 50, from
...
- [ ] B4 🧠 **Review checkpoint.** Strongest model reviews Phase B diff (`git diff v0-baseline`)
      for: polling leaks (intervals not cleared), duplicate fetch storms, PnL math errors.
      Fix findings. Tag `v1-dashboard`.

## Phase C — Port wallet money panels (browser stays key-free)

- [ ] C1 🧠 **Design the lean wallet-data hook.** Read `crypto-wallet/src/context/
      ...
      No signing code may be ported — AD-2.
- [ ] C2 🔧 **Implement `useWalletData` hook + Wallet panel on Dashboard** per C1 note:
...
- [ ] C3 🧠 **Manual trade via engine.** Add a "Quick Trade" box (symbol, side, USD) that
...
- [ ] C4 🧠 **Review checkpoint.** Tag `v2-wallet-panels`.

## Phase D — Retire the old wallet app + in-browser auto-trade
...
- [ ] E1 🔧 App title/branding "Alpha Terminal", favicon, consistent dark theme spacing;
      Dashboard is default tab (done in B1), Charts/Strategies/Finder/Settings order.
- [ ] E2 🔧 Settings tab: engine risk limits editor (port of wallet ConfigPanel, PATCH
      `/engine/settings`) + collector/API health detail.
- [ ] E3 🧠 UX pass with the user: naming, empty states, confirmation dialogs for LIVE.
      Tag `v4-alpha-terminal`.
